...
def point_cloud_masking_rough(pts, logits):     # 测一下如果加入球查询效果会不会使质心更准，先粗略找到物体中心，再补充新的点，再次找准确中心
    # Binary Classification for each point
    mask = logits[:, :, 0] < logits[:, :, 1]  # (bs, n)
    mask = mask.unsqueeze(1).float()  # (bs, 1, n) 在下标为1的地方加一个维度
    mask_count = mask.sum(2, keepdim=True).repeat(1, 3, 1)  # (bs, 3, 1)
    pts_xyz = pts[:, :3, :]  # (bs,3,n)     如果是包含法向量的6个维度就只提取xyz三个维度
    dim4 = pts[:, 3:4, 0:1].transpose(2,1)
    mask_xyz_mean = (mask.repeat(1, 3, 1) * pts_xyz).sum(2, keepdim=True)  # (bs, 3, 1)
    mask_xyz_mean = mask_xyz_mean / torch.clamp(mask_count, min=1)  # (bs, 3, 1) clamp将输入张量每个元素的值压缩到区间[min,max]
    mask_xyz_mean = torch.cat([mask_xyz_mean.transpose(2,1), dim4], 2).transpose(2,1)
    return mask_xyz_mean


# 三次坐标系变换：frustum coordinate——>3D mask coordinate——>3D object coordinate
...
class FrustumPointNetLoss(nn.Module):
    ...
    def forward(self, logits_old, logits_new, mask_label, mask_label_new, \
        ...
        bs = logits_new.shape[0]
        # 3D Instance Segmentation PointNet Loss
        logits_old = F.log_softmax(logits_old.view(-1, 2), dim=1)  # torch.Size([32768, 2])
        mask_label = mask_label.view(-1).long()  # torch.Size([32768])
        mask_loss_old = F.nll_loss(logits_old, mask_label)

        logits_new = F.log_softmax(logits_new.view(-1,2),dim=1)#torch.Size([32768, 2])
        mask_label_new = mask_label_new.view(-1).long()#torch.Size([32768])
        mask_loss_new = F.nll_loss(logits_new, mask_label_new)#tensor(0.6361, grad_fn=<NllLossBackward>)

        # Center Regression Loss
        center_dist = torch.norm(center-center_label,dim=1)#(32,)
        center_loss = huber_loss(center_dist, delta=2.0)

        stage1_center_dist = torch.norm(center-stage1_center,dim=1)#(32,)   # 为什么质心的loss也要和center比？
        ...
        total_loss = mask_loss_old + mask_loss_new + box_loss_weight * (center_loss + \
        ...
        ###    ipdb.set_trace()
        if self.return_all:
            return total_loss, mask_loss_new, \
