...
def rotate_pc_along_y(pc, rot_angle):
    ...
    pc[:, [0, 2]] = np.dot(pc[:, [0, 2]], np.transpose(rotmat))     # 绕y轴转，y坐标不变
    return pc
...
def size2class(size, type_name):
    ...
    '''
    size_class = g_type2class[type_name]    # 取出类型的序号
    size_residual = size - g_type_mean_size[type_name]      # 与包围盒平均大小（长宽高）的差
...
class FrustumDataset(object):
    ...
    def __init__(self, npoints, split,
        ...
        Input:
            ...
            random_flip: bool, in 50% randomly flip the point cloud         翻转
        ...
        else:   # 按照prepare_data.py中生成的pickle文件格式，依次将内容读出来
            with open(overwritten_data_path, 'rb') as fp:
                # self.id_list = pickle.load(fp)          # [3, 3, 3, 3, 3, 7, 7, 7, 7, 7, ...]
                # self.box2d_list = pickle.load(fp)       # [array([353.50785453, 180.46926429, 469.4132269 , 213.90955288]),...]
                self.box3d_list = pickle.load(fp)       # 同上八个点坐标
                self.input_list = pickle.load(fp)       # pc_in_box_fov  ex:(59205 * 1147 * 4)
                self.label_list = pickle.load(fp)       # (1147)
                self.type_list = pickle.load(fp)        # ['Car', 'Car', ...]
                self.heading_list = pickle.load(fp)     # [1.62, 1.62, 1.62, 1.62, 1.62, -1.59, -1.59, -1.59, -1.59, -1.59, ...]
                self.size_list = pickle.load(fp)        # [array([4.29, 1.68, 1.42]), array([3.8 , 1.4 , 1.36]),...]
                # frustum_angle is clockwise angle from positive x-axis
                self.frustum_angle_list = pickle.load(fp)   # [-1.4360755977657527, -1.4411391593588625,
                # self.calib_list = pickle.load(fp)       # 没用？？保存的时候可以删掉
                self.origin_pts_list = pickle.load(fp)
                self.label_full_list = pickle.load(fp)
    ...
    def __getitem__(self, index):
        ...
        if self.one_hot:  # True
            cls_type = self.type_list[index]    # 'Car'
            assert (cls_type in ['Car', 'Pedestrian', 'Cyclist'])
            one_hot_vec = np.zeros((3))
            one_hot_vec[g_type2onehotclass[cls_type]] = 1       # 1,0,0代表Car, 010代表Pedestrian, 001代表Cyclist

        # Get point cloud
        if self.rotate_to_center:  # True       将整个rect坐标系旋转至z轴与截头体方向对齐（涉及的点跟着旋转）
            point_set = self.get_center_view_point_set(index)  # (n,4) #pts after Frustum rotation  只包含小截头体内的点
            origin_point_set = np.copy(self.origin_pts_list[index])
            origin_point_set = rotate_pc_along_y(origin_point_set, self.get_center_view_rot_angle(index))
        else:
        ...
        # choice2 = np.random.choice(origin_point_set.shape[0], 3072, replace=True)
        # origin_point_set = origin_point_set[choice2, :]
        ...
        label_full = self.label_full_list[index]
        # label_full = label_full[choice]
        ...
        if self.rotate_to_center:  # True
            heading_angle = self.heading_list[index] - rot_angle  # -1.6480684951683866 #就是alpha（=ry+theta）
        else:
        ...
        if self.one_hot:
            return point_set, seg, box3d_center, angle_class, angle_residual, \
                   size_class, size_residual, rot_angle, one_hot_vec, origin_point_set, label_full
        else:
            return point_set, seg, box3d_center, angle_class, angle_residual, \
    ...
    def get_center_view_box3d_center(self, index):
        ...
        box3d_center = (self.box3d_list[index][0, :] + \
                        self.box3d_list[index][6, :]) / 2.0     # 对角线中点
        return rotate_pc_along_y(np.expand_dims(box3d_center, 0), \
                                 self.get_center_view_rot_angle(index)).squeeze()       # squeeze()降维，去除是1的维度
