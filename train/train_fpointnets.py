...
def test_one_epoch(model, loader):
    ...
    for i, data in tqdm(enumerate(loader), \
        ...
        batch_rot_angle, batch_one_hot_vec, origin_pts, label_full = data
        ...
        origin_pts = origin_pts.float().cuda()
        label_full = label_full.float().cuda()

        model = model.eval()

        logits_old, logits_new, mask, stage1_center, center_boxnet, \
        ...
        size_scores, size_residuals_normalized, size_residuals, center, additional_label = \
            model(origin_pts, label_full, batch_data, batch_one_hot_vec)

        additional_label = additional_label.float().cuda()
        batch_label_new = torch.cat([batch_label, additional_label], 1)

        logits_old = logits_old.detach()
        logits_new = logits_new.detach()
        ...
        batch_label_new = batch_label_new.detach()
        ...
        if FLAGS.return_all_loss:
            total_loss, mask_loss, center_loss, heading_class_loss, \
                ...
                corners_loss = \
                Loss(logits_old, logits_new, batch_label, batch_label_new, \
        ...
        correct = torch.argmax(logits_new, 2).eq(batch_label_new.detach().long()).cpu().numpy()
...
def train():
    ...
    start= time.perf_counter()      # 计时器从0开始
    SEED = 1
    np.random.seed(SEED)
    torch.manual_seed(SEED)
    torch.cuda.manual_seed_all(SEED)
    random.seed(SEED)
    torch.backends.cudnn.deterministic = True       # 目的与SEED相同，保证每次运行网络的时候相同输入的输出是固定的
    ...
    if os.path.exists('runs/' + NAME):
        ...
        shutil.rmtree('runs/' + NAME)   # 递归地删除文件夹以及里面的文件
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for i, data in tqdm(enumerate(train_dataloader),\
            ...
            n_samples += data[0].shape[0]       # 每次加32(batchsize)      data[0]->[32, 1024, 4]
            #for debug
            if FLAGS.debug==True:
                if i==1 :
                    break
            ...
            '''  
            point_set, seg, box3d_center, angle_class, angle_residual, size_class, size_residual, rot_angle, one_hot_vec
            
            data after frustum rotation     data中的9个tensor数据(dataloader.__getitem__()返回值)，均以32(batchsize)为第一维
            1. For Seg
            batch_data:[32, 2048, 4], pts in frustum, 
            batch_label:[32, 2048], pts ins seg label in frustum, 全是01（1的下标代表属于物体部分的点下标）
            2. For T-Net
            batch_center:[32, 3],
            3. For Box Est.
            batch_hclass:[32],
            ...
            batch_data = batch_data.transpose(2,1).float().cuda()   # torch的网络输入要求通道数在中间
            batch_label = batch_label.float().cuda()
            batch_center = batch_center.float().cuda()
            ...
            batch_one_hot_vec = batch_one_hot_vec.float().cuda()
            origin_pts = origin_pts.float().cuda()      # bs, m, 4      m = 7168
            label_full = label_full.float().cuda()      # B, 7168
            ...
            heading_scores, heading_residuals_normalized, heading_residuals, \
            size_scores, size_residuals_normalized, size_residuals, center, additional_label = \
                FrustumPointNet(origin_pts, label_full, batch_data, batch_one_hot_vec)

            # batch_label与additional_label拼接（batch_hclass, batch_hres等需不需要重新计算？）
            additional_label = additional_label.float().cuda()
            batch_label_new = torch.cat([batch_label, additional_label], 1)     # B, 2048
            batch_label_new.float().cuda()
            ...
            if FLAGS.return_all_loss:
                total_loss, mask_loss, center_loss, heading_class_loss, \
                    ...
                    size_residuals_normalized_loss, stage1_center_loss, \
                    corners_loss = \
                    Loss(logits_old, logits_new, batch_label, \
                        center, batch_center, stage1_center, \
                        heading_scores, heading_residuals_normalized, \
                        heading_residuals, \
                        batch_hclass, batch_hres, \
            ...
            iou2ds, iou3ds = provider.compute_box3d_iou(\
                ...
                batch_hres.cpu().detach().numpy(), \
                batch_sclass.cpu().detach().numpy(), \
                batch_sres.cpu().detach().numpy())      # 使用detach()不计算梯度并阻止梯度传递
            train_iou2d += np.sum(iou2ds)
            train_iou3d += np.sum(iou3ds)
            train_iou3d_acc += np.sum(iou3ds>=0.7)

            correct = torch.argmax(logits_new, 2).eq(batch_label_new.long()).detach().cpu().numpy()
        ...
        # 保存最优模型
        if test_iou3d_acc >= best_iou3d_acc:        # 最优模型标准
            best_iou3d_acc = test_iou3d_acc
            best_epoch = epoch + 1
            if epoch > MAX_EPOCH / 5:   # 从30epoch以后开始保存最优模型
                ...
                bestfile = savepath     # 这里写少了下划线，导致每个最优模型全部被保存而不是替换，问题不大
                state = {
                    'epoch': epoch + 1,
