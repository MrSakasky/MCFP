...
def get_lidar_index_in_image_fov(pc_velo, calib, xmin, ymin, xmax, ymax, return_more=False, clip_distance=2.0):
    """ Filter lidar points, keep those in image FOV """
    pts_2d = calib.project_velo_to_image(pc_velo)
    fov_inds = (
        (pts_2d[:, 0] < xmax)
...
def show_lidar_with_depth(
    ...
    for obj in objects:
        ...
        #TODO: change the color of boxes
        if obj.type == "Car":
            draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=(0,1,0), label=obj.type)    # label修改
        elif obj.type == "Pedestrian":
    ...
    mlab.show()   # mlab.show(1)有问题


def save_depth0(data_idx, pc_velo, calib, img_fov, img_width, img_height, depth, constraint_box=False):
...
def save_depth(data_idx, pc_velo, calib, img_fov, img_width, img_height, depth, constraint_box=False):
    if depth is not None:
        depth_pc_velo = calib.project_depth_to_velo(depth, constraint_box)
...
def dataset_viz(root_dir, args):
    dataset = kitti_object(root_dir, split=args.split, args=args)   # 要自己设定长度
    ...
    if args.show_lidar_with_depth:
        ...
        fig = mlab.figure(
            figure=None, bgcolor=(0, 0, 0), fgcolor=None, engine=None, size=(1000, 500)     # 修改背景颜色
        )
    for data_idx in range(len(dataset)):    # 7481
        if args.ind > 0:
            data_idx = args.ind
